...
- 토크나이저()
  - 토크나이저는 일반적으로 공백(탭, 공백, 새 줄)을 찾아 텍스트 스트림을 토큰으로 나눈다.
  - 간단하게 말하면 Tokenization이란 Text를 여러개의 Token으로 나누는 것을 말한다.
  - Tokenizer 별로 Token을 생성하는 방식이 다르기 때문에 어떤 데이터를 다루냐에 따라 다른 Tokenizer를 사용해야 한다.
- 렉서(lexer, 낱말분석)
  - 렉서는 lexical analysis를 한다. 직역하면 '어휘 분석' 이다. 좀 더 풀어서 설명하면 렉서는 텍스트를 받아서 한 글자 한 글자 읽어나가다가 의미를 가진 단어를 만나면, 그 단어를 전체 텍스트로부터 잘라서 `token`이란 것으로 만든다.
- 파서
  - 파서는 렉서에서 토큰 스트림을 가져와 원본 텍스트가 나타내는 (일반적으로) 프로그램을 나타내는 추상 구문 트리로 바꾼다.
  - 컴퓨터 과학에서 파싱((syntactic) parsing)은 일련의 문자열을 의미있는 토큰(token)으로 분해하고 이들로 이루어진 파스 트리(parse tree)를 만드는 과정을 말한다.

## 6. 트리(Tree) - 이진 트리

- 트리는 연결 리스트(Linked list)와 비슷하나, 연결 리스트는 다른 노드로 가는 링크가 한 개인 것에 반해 트리는 여러 개의 노드들로 연결되는 것이 가능하다.
- 트리는 데이터를 계층적으로 저장하는 비연속 형식의 자료구조로, 파일시스템에 파일을 저장하거나 정렬된 데이터 리스트 등 계층적인 데이터를 저장할 때 사용된다. 

### 6.1 트리 정의

- 트리는 `에지(edge)`로 구성된 `노드(node)`의 집합이다. 
- 트리를 구성하는 항목
  - 트리의 최상위 노드를 `루트(root) 노드` 라고 한다.
  - 한 노드가 아래 노드로 연결되어 있을 때 위에 있는 노드를 `부모(parent) 노드`라고 한다.
  - 부모 노드의 아래에 있는 모든 노드를 `자식(child) 노드`라고 한다. 
    - 한 노드는 0개 이상의 노드와 연결될 수 있다.
  - 자식 노드가 없는 노드를 `리프(leaf) 노드`라 부른다.
- 이진트리
  - 이진 트리는 모든 노드의 자식 노드 수가 2개 이하인 특수한 노드를 가리킨다.
  - 이진 노드는 몇 가지 동작을 매우 효율적으로 수행할 수 있다는 특징이 있다.
- 직접 연결되지 않은 노드라도 연결된 `에지`를 이용해 방문할 수 있다.
  - 한 노드에서 다른 노드로 도달하는데 사용한 에지의 모음을 `경로(path)`라 한다.
- 트리의 모든 노드를 일정한 순서로 방문하는 것을 `트리 탐색(tree traversal)`이라 한다. 
- 트리 레벨
  - 트리는 레벨로 구분할 수 있다. 루트 노드는 레벨 0의 위치에 있으며 루트 노드의 자식은 레벨1, 자식의 자식은 레벨이로 구성된다.
  - 트리에 있는 레벨의 수를 `트리의 깊이(depth)`라 한다.
- 트리의 루트는 실제 나무의 뿌리와 달리 위에 있다.
- 트리의 각 노드는 값을 갖는데, 노드의 값을 `키 값`이라고도 표현한다.
